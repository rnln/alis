...
alias ls="${__MACOS_GNU_PREFIX}ls --hyperlink --color"
...
function accessed () { emulate -L zsh; print -l -- *(a-${1:-1}D) }
function changed  () { emulate -L zsh; print -l -- *(c-${1:-1}D) }
function modified () { emulate -L zsh; print -l -- *(m-${1:-1}D) }
...
function pwdgen () {
	...
	local password=$(LC_CTYPE=C gtr -dc a-z </dev/urandom | head -c $lowercase_length)
	password=$password$(LC_CTYPE=C gtr -dc "\-/." </dev/urandom | head -c 1)
	password=$password$(LC_CTYPE=C gtr -dc 0-9    </dev/urandom | head -c 1)
	password=$password$(LC_CTYPE=C gtr -dc A-Z    </dev/urandom | head -c 1)
	password=$(echo $password | grep -o . | sort -R | gtr -d "\n" | head -c $length)
	echo $password
}
...
function autoclean () {
	rm "$HOME"/.v8flags.*.json
}

function tony-master () {
	local return_code=0
	if [[ -z "$1" ]] ; then
